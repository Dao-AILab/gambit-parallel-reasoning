...
class CompletionOutput:
    ...
    # Gambit: Score history for trajectory analysis
    score_history: list[float] | None = None
    # Gambit branch metadata
    gambit_parent_id: str | None = None
    gambit_branch_depth: int = 0
    gambit_is_branch: bool = False
    ...
    def __repr__(self) -> str:
        ...
        if self.score_history:
            repr_str += f", score_history=[{len(self.score_history)} scores]"
        if self.gambit_is_branch:
            repr_str += (
                f", gambit_parent_id={self.gambit_parent_id}, "
                f"gambit_branch_depth={self.gambit_branch_depth}"
            )
        return repr_str + ")"
