...
def fast_plan_decode(
    ...
    if not self.is_cuda_graph_enabled or getattr(self, "vllm_first_call", True):
        # NOTE(rebuttal/flashinfer-fix 2026-05-26): flashinfer >= 0.6 inserted
        # an extra `o_data_type` parameter into `plan(...)` between
        # `kv_data_type` and `data_type`. The previous positional call here
        # mis-aligned every subsequent argument, ending with
        # `non_blocking=None` reaching torch.Tensor.to() and raising
        # `TypeError: to() received an invalid combination of arguments`.
        # Use keyword args so we are robust to such signature drift.
        self.plan(
            indptr_cpu,
            indices,
            last_page_len_cpu,
    ...
    try:
        # NOTE(rebuttal/flashinfer-fix 2026-05-26): flashinfer >= 0.6 added a
        # trailing `num_colocated_ctas` int arg to the tensor-core decode plan
        # C++ signature (see flashinfer/decode.py upstream "args.append(0)
        # # num_colocated_ctas"). vLLM v0.11.1's positional call was 18 args;
        # the kernel now expects 19. Mirror the upstream default of 0.
        self._plan_info = self._cached_module.plan(
            ...
            fixed_split_size,
            disable_split_kv,
            0,  # num_colocated_ctas (added in flashinfer >= 0.6)
        )
