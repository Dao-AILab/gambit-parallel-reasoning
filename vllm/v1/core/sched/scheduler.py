...
# scheduler-level timing (runs in same process as EngineCore).
_sched_perf: dict[str, dict] = defaultdict(
    lambda: {"n": 0, "total_ms": 0.0, "max_ms": 0.0}
)

@contextmanager
def _sched_timed(phase: str):
    # time a scheduler code region.
...
def _sched_perf_flush() -> None:
    # write scheduler snapshot to JSONL.
    path = os.environ.get("GAMBIT_SCHED_PROFILE_PATH", "")
    if not path:
        return
    try:
        with open(path, "a") as _pf:
            _pf.write(json.dumps({k: dict(v) for k, v in _sched_perf.items()}) + "\n")
    except Exception:
        pass
...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        self.gambit_config = getattr(vllm_config, "gambit_config", None)
        ...
        if self.gambit_config is not None and self.gambit_config.enable:
            requested_policy = SchedulingPolicy.GAMBIT.value
        try:
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        if self.policy == SchedulingPolicy.GAMBIT:
    ...
    def _order_running_by_score(self) -> None:
        if not self.running:
            return
        # order_running_by_score: O(N log N) sort fired every
        # scheduler step when Gambit policy is active; cost scales with running queue size.
        with _sched_timed("order_running_by_score"):
            self.running.sort(
    ...
    def _stop_lowest_running(self) -> Request | None:
        ...
        logger.debug(
            "[Gambit] stopping request %s at tokens=%d, output_tokens=%d",
            victim.request_id,
            victim.num_tokens,
    ...
    def update_from_output(
        ...
        trace_score_histories: dict[str, list[float]] | None = None,
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            # Get score history for trajectory analysis
            score_history = (
                trace_score_histories.get(req_id)
                if trace_score_histories is not None and finish_reason is not None
                else None
            )
