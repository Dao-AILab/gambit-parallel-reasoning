...
from vllm.v1.hiddenstate.classifier import HiddenstateClassifier, SequenceScorer
...
# module-level accumulator for GPU worker timing.
# Written to GAMBIT_WORKER_PROFILE_PATH (separate from engine-core profile).
_worker_perf: dict[str, dict] = defaultdict(
    lambda: {"n": 0, "total_ms": 0.0, "max_ms": 0.0}
)
_worker_perf_flush_counter: int = 0

@contextmanager
def _worker_timed(phase: str):
    # time a code region inside the GPU worker process.
    t0 = time.perf_counter()
    try:
        yield
    finally:
        dur_ms = (time.perf_counter() - t0) * 1000.0
        p = _worker_perf[phase]
        p["n"] += 1
        p["total_ms"] += dur_ms
        if dur_ms > p["max_ms"]:
            p["max_ms"] = dur_ms

def _worker_perf_flush() -> None:
    # append snapshot to worker profile JSONL.
    global _worker_perf_flush_counter
    path = os.environ.get("GAMBIT_WORKER_PROFILE_PATH", "")
    if not path:
        return
    snapshot = {
        "flush_idx": _worker_perf_flush_counter,
        "pid": os.getpid(),
        "phases": {k: dict(v) for k, v in _worker_perf.items()},
    }
    _worker_perf_flush_counter += 1
    try:
        with open(path, "a") as _pf:
...
class GPUModelRunner(
    ...
    def __init__(
        ...
        self.step_scorer: HiddenstateClassifier | SequenceScorer | None = None
        self._is_sequence_scorer: bool = False
        # Per-request hidden state history for the sequence scorer (accumulates across rounds).
        self._all_hidden_states_by_req: dict[str, list[torch.Tensor]] = {}
        self.load_step_scorer_checkpoint(
            model_path=self.vllm_config.gambit_config.step_scorer_path
        )

        # Double-buffered scorer with CUDA stream overlap
    ...
    def load_step_scorer_checkpoint(self, model_path: str) -> None:
        ...
        is_sequence = (
            model_config.get('type') == 'sequence_transformer'
            or model_config.get('arch') == 'encoder'
            or 'sequence' in os.path.basename(model_path).lower()
        )
        if is_sequence:
            cfg = model_config
            model = SequenceScorer(
                input_dim=cfg.get('input_dim', self.hidden_size),
                d_model=cfg.get('d_model', 256),
                nhead=cfg.get('nhead', 4),
                dim_feedforward=cfg.get('dim_feedforward', 0),
                dropout=cfg.get('dropout', 0.1),
                max_len=cfg.get('max_len', 4096),
                num_layers=cfg.get('num_layers', 1),
                rope_base=cfg.get('rope_base', 10000),
            ).to(self.device)
            self._is_sequence_scorer = True
        else:
            hidden_dims = None
            if model_config.get('arch') == 'mlp':
                layers_n = int(model_config.get('mlp_layers', 2))
                hb = int(model_config.get('mlp_hidden', 512))
                if layers_n == 2:
                    hidden_dims = [hb]
                elif layers_n == 3:
                    hidden_dims = [hb * 2, hb]
                elif layers_n == 4:
                    hidden_dims = [hb * 4, hb * 2, hb]
            model = HiddenstateClassifier(
                input_dim=self.hidden_size, hidden_dims=hidden_dims
            ).to(self.device)
            self._is_sequence_scorer = False
    ...
    def step_scorer_evaluate(self, req_ids, num_scheduled_tokens):
        ...
        with torch.inference_mode():
            for rid, t in hs.items():
                t_dev = t.to(device=self.device, dtype=torch.float32, non_blocking=True)
                if self._is_sequence_scorer:
                    hist = self._all_hidden_states_by_req.setdefault(rid, [])
                    hist.append(t_dev)
                    # sequence_cat: O(T) torch.cat grows with trace length;
                    # called once per scored request per scoring round.
                    with _worker_timed("sequence_cat"):
                        full_seq = torch.cat(hist, dim=0).unsqueeze(0)  # [1, T_full, D]
                    logits = self.step_scorer(full_seq)              # [1, T_full, 1]
                    # scorer_item: GPU→CPU sync to extract scalar score.
                    with _worker_timed("scorer_item"):
                        score = torch.sigmoid(logits[0, -1, 0]).item()
                    results[rid] = [score]
                    _worker_perf_flush()
                else:
                    results[rid] = torch.sigmoid(
                        self.step_scorer(t_dev)
                    ).flatten().tolist()
        return results

    def step_scorer_evaluate_with_hidden_states(self, req_ids, num_scheduled_tokens):
        hs = self.get_hidden_states_by_req(req_ids, num_scheduled_tokens)
        if self.step_scorer is None:
            raise RuntimeError("classifier not loaded")
        outputs = {}
        with torch.inference_mode():
            for rid, t in hs.items():
                ...
                if self._is_sequence_scorer:
                    hist = self._all_hidden_states_by_req.setdefault(rid, [])
                    hist.append(t_dev)
                    # sequence_cat (with_hs variant): same O(T) cost.
                    with _worker_timed("sequence_cat_with_hs"):
                        full_seq = torch.cat(hist, dim=0).unsqueeze(0)
                    logits = self.step_scorer(full_seq)
                    # scorer_item (with_hs variant).
                    with _worker_timed("scorer_item_with_hs"):
                        score = torch.sigmoid(logits[0, -1, 0]).item()
                    scores = [score]
                else:
                ...
                # Keep only the latest hidden state snapshot for on-policy mining.
                latest = t[-1] if t.ndim > 1 else t
                # cpu_transfer: GPU→CPU copy of full hidden state
                # vector (~4096 float16 values); called once per scored request when mining is on.
                with _worker_timed("cpu_transfer"):
                    hs_cpu = latest.detach().to(dtype=torch.float16, device="cpu").tolist()
                outputs[rid] = {
                    "scores": scores,
                    "hidden_state": hs_cpu,
                }
    ...
    def sync_sequence_histories(
        self,
        clear_ids: list[str],
        copy_pairs: list[tuple[str, str]],
    ) -> None:
        """Clear history for pruned requests and copy history for new branches."""
        if not self._is_sequence_scorer:
            return
        for rid in clear_ids:
            self._all_hidden_states_by_req.pop(rid, None)
        for parent_id, child_id in copy_pairs:
            if parent_id in self._all_hidden_states_by_req:
                self._all_hidden_states_by_req[child_id] = list(
                    self._all_hidden_states_by_req[parent_id]
                )

    def enable_hidden_states_capture(self) -> None:
        self.vllm_config.gambit_config.enable = True

    def disable_hidden_states_capture(self) -> None:
        self.vllm_config.gambit_config.enable = False
    ...
    def get_hidden_states_debug_info(self) -> dict[str, int]:
        return {
            "enabled": int(self.vllm_config.gambit_config.enable),
    ...
    def _bookkeeping_sync(
        ...
        if (
            ...
            and self.vllm_config.gambit_config.enable
    ...
    def sample_tokens(
        ...
        if self.vllm_config.gambit_config.enable and valid_sampled_token_ids:
            scoring_stride = self.vllm_config.gambit_config.scoring_stride
            if scoring_stride > 0:
                # Stride mode: capture hidden states for ALL requests on every
                # forward pass so they are available when the engine fires a
                # stride-based scoring trigger.
                trigger_req_ids = list(req_id_to_index_output_copy.keys())
            else:
                # Default: capture only when \n\n is sampled.
                trigger_req_ids = []
                for rid, idx in req_id_to_index_output_copy.items():
                    ...
                    boundary_ids = self.vllm_config.gambit_config.double_new_line_tokenID
                    if boundary_ids and any(t in boundary_ids for t in tokens):
