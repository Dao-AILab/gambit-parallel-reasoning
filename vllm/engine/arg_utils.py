...
class EngineArgs:
    ...
    gambit_enable: bool = GambitConfig.enable
    gambit_step_scorer_path: str | None = GambitConfig.step_scorer_path
    gambit_stop_thinking_tokenID: int | None = GambitConfig.stop_thinking_tokenID
    gambit_double_new_line_tokenID: list[int] | None = GambitConfig.double_new_line_tokenID
    gambit_scoring_stride: int = GambitConfig.scoring_stride
    gambit_record_score_history: bool = GambitConfig.record_score_history
    gambit_collect_hidden_states_for_mining: bool = (
        GambitConfig.collect_hidden_states_for_mining
    )
    # Gambit config
    gambit_enable_branching: bool = GambitConfig.enable_branching
    # Stop criterion: if > 0, stop generation once this many traces have completed
    gambit_stop_after_completed_traces: int = GambitConfig.stop_after_completed_traces
    # Tournament mode (recommended)
    gambit_tournament_mode: bool = GambitConfig.tournament_mode
    gambit_tournament_capacity: int = GambitConfig.tournament_capacity
    gambit_tournament_swap_k: int = GambitConfig.tournament_swap_k
    gambit_tournament_check_interval: int = GambitConfig.tournament_check_interval
    gambit_tournament_warmup_tokens: int = GambitConfig.tournament_warmup_tokens
    gambit_tournament_hard_floor: float = GambitConfig.tournament_hard_floor
    gambit_tournament_branch_random: bool = GambitConfig.tournament_branch_random
    # Legacy threshold mode
    gambit_theta_branch: float = GambitConfig.theta_branch
    gambit_theta_prune: float = GambitConfig.theta_prune
    gambit_max_branches_per_trace: int = GambitConfig.max_branches_per_trace
    # NOTE: initial_traces is determined by sampling_params.n (--num-traces)
    gambit_max_total_traces: int = GambitConfig.max_total_traces
    gambit_branch_cooldown_steps: int = GambitConfig.branch_cooldown_steps
    gambit_branch_temperature_boost: float = GambitConfig.branch_temperature_boost
    gambit_min_tokens_before_branch: int = GambitConfig.min_tokens_before_branch
    gambit_min_tokens_before_prune: int = GambitConfig.min_tokens_before_prune
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        gambit_kwargs = get_kwargs(GambitConfig)
        gambit_group = parser.add_argument_group(
            title="GambitConfig",
            description=GambitConfig.__doc__,
        )
        gambit_group.add_argument(
            "--gambit-enable", **gambit_kwargs["enable"]
        )
        gambit_group.add_argument(
            "--gambit-step-scorer-path", **gambit_kwargs["step_scorer_path"]
        )
    ...
    def create_engine_config(
        ...
        # Gambit config (including Gambit parameters)
        # NOTE: initial_traces is determined by sampling_params.n (--num-traces)
        gambit_config = GambitConfig(
            enable=self.gambit_enable,
            step_scorer_path=self.gambit_step_scorer_path,
            scoring_stride=self.gambit_scoring_stride,
            record_score_history=self.gambit_record_score_history,
            collect_hidden_states_for_mining=self.gambit_collect_hidden_states_for_mining,
            enable_branching=self.gambit_enable_branching,
            stop_after_completed_traces=self.gambit_stop_after_completed_traces,
            # Tournament mode (recommended)
            tournament_mode=self.gambit_tournament_mode,
            tournament_capacity=self.gambit_tournament_capacity,
            tournament_swap_k=self.gambit_tournament_swap_k,
            tournament_check_interval=self.gambit_tournament_check_interval,
            tournament_warmup_tokens=self.gambit_tournament_warmup_tokens,
            tournament_hard_floor=self.gambit_tournament_hard_floor,
            tournament_branch_random=self.gambit_tournament_branch_random,
            # Legacy threshold mode
            theta_branch=self.gambit_theta_branch,
            theta_prune=self.gambit_theta_prune,
            max_branches_per_trace=self.gambit_max_branches_per_trace,
            max_total_traces=self.gambit_max_total_traces,
            branch_cooldown_steps=self.gambit_branch_cooldown_steps,
            branch_temperature_boost=self.gambit_branch_temperature_boost,
            min_tokens_before_branch=self.gambit_min_tokens_before_branch,
            min_tokens_before_prune=self.gambit_min_tokens_before_prune,
        )
