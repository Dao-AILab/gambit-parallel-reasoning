...
class LLM:
    ...
    def get_gambit_tree_export(self) -> dict | None:
        """
        Get the Gambit trace tree export.

        Returns a dictionary containing:
        - stats: Overall tree statistics (active/completed/pruned counts, etc.)
        - nodes: Dict of all trace nodes with their metadata
        - roots: List of root request IDs

        Returns None if Gambit branching is not enabled.
        """
        if hasattr(self.llm_engine, 'get_gambit_tree_export'):
            return self.llm_engine.get_gambit_tree_export()
        return None

    def reset_gambit_state(self) -> None:
        """Reset Gambit state between independent generate() calls."""
        if hasattr(self.llm_engine, "reset_gambit_state"):
            self.llm_engine.reset_gambit_state()
    ...
    def _run_engine(
        ...
        # If Gambit branching is enabled, the engine core may still have active
        # internally-created branch requests even after the initial requests
        # (tracked by the frontend) are finished. In that case, keep stepping
        # the engine until the Gambit trace tree reports no active traces.
        #
        # This ensures `LLM.generate()` does not return early and that callers
        # can rely on `get_gambit_tree_export()` to include completed branches.
        # The drain is guarded on `scheduler_active_count` (= len(waiting) +
        # len(running)), not on the tree's `active_count`. Ghost traces stay
        # logically active while holding no memory, so `active_count` never
        # reaches zero and would deadlock the loop.
        def _drain_counts() -> tuple[int, int] | None:
            export = self.get_gambit_tree_export()
            if not isinstance(export, dict):
                return None
            stats = export.get("stats") or {}
            active = int(stats.get("active_count", 0) or 0)
            return active, int(stats.get("scheduler_active_count", active) or 0)

        try:
            counts = _drain_counts()
            sched_count = counts[1] if counts else 0
            while sched_count > 0:
                # Keep consuming engine outputs to allow internal requests to
                # make progress. These outputs may not correspond to any
                # frontend-tracked RequestState, so we intentionally ignore
                # `step_outputs` here.
                self.llm_engine.step()
                counts = _drain_counts()
                if counts is None:
                    # If export is unavailable, don't risk blocking forever.
                    logger.warning(
                        "Gambit trace-tree export unavailable; ending drain loop."
                    )
                    break
                sched_count = counts[1]
        except Exception:
            logger.exception(
                "Gambit drain loop failed; returning outputs generated so far."
            )
